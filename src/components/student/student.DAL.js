...
export async function studentFindByIdAndUpdate(studentId, updateObj) {
  try {
    return await Student.findByIdAndUpdate(studentId, updateObj, {
      new: true,
    }).lean();
  } catch (err) {
    throw new HttpException(
      500,
      STUDENT_ERROR_CODES.UPDATE_STUDENT_UNHANDLED_IN_DB,
...
export async function deleteStudentById(studentId) {
  try {
    ...
    throw new HttpException(
      500,
      STUDENT_ERROR_CODES.DELETE_STUDENT_UNHANDLED_IN_DB,
...
export async function batchAggregate(agg) {
  try {
    ...
    throw new HttpException(
      500,
      STUDENT_ERROR_CODES.STUDENT_AGG_UNHANDLED_IN_DB,
...
export async function attandanceAggregate(agg) {
  try {
    ...
    throw new HttpException(
      500,
      STUDENT_ERROR_CODES.ATT_STUDENT_AGG_UNHANDLED_IN_DB,
...
export async function countDocs(filterData) {
  try {
    ...
    throw new HttpException(
      500,
      STUDENT_ERROR_CODES.STUDENT_COUNTDOC_UNHANDLED_IN_DB,
