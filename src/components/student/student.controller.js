...
class StudentsController {
  ...
  async addStudent(req, res, next) {
    try {
      ...
      const { name, emailId, mobileNo, department, batch, currentSem } =
        req.body;
      const studentObject = {
        name,
        emailId,
  ...
  async studentsVacancy(req, res, next) {
    if (!req.params.batchId) {
      throw new HttpException(
        404,
        STUDENT_ERROR_CODES.BAD_REQUEST_STUDENTS,
        "BAD_REQUEST_STUDENTS",
        null
      );
    }
  ...
  async attendanceStudents(req, res, next) {
    ...
    if (totalDays === 0) {
      throw new HttpException(
        404,
        STUDENT_ERROR_CODES.NO_ATTENDANCE_STUDENTS,
    ...
    const agg = [
      ...
      },
      {
        $match: {
          $expr: {
            $lt: ["$totalPresence", seventyFivePercent],
          },
        },
      },
      {
